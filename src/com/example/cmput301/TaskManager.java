...
public class TaskManager {
	...
	private static final int UPDATE_WEBSERVICE = 1;
	Task someTask;
	Task somenewTask;
	public Task shareTask(String id) {
		...
		new ShareTask().execute(localTask);
	...
	private class ShareTask extends AsyncTask<Task,Void,Task>{

		Task localTask;

		@Override
		protected Task doInBackground(Task... args)
		{
			if(args.length > 0)
			{
				localTask = args[0];
				return WebService.put(localTask);
			}
			return null;
		}
	...
	public void setTask(Task result, Task localTask)
	{
		// TODO Auto-generated method stub
		Log.d("Task",result.getName());

		//Add the remote version of the task.
		result.setStatus(Task.STATUS_SHARED);
		Task returnedTask = dbman.postLocal(result);
		if(returnedTask.getId()==result.getId())
		{
			Log.d("Task","Added properly");
